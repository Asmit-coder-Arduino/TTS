...
import { CharacterUsageStats } from "../types";
import { formatCharacterCount, getUsagePercentage, getUsageStatus } from "../lib/wordCounter";

interface CharacterUsageDisplayProps {
  usage: CharacterUsageStats;
  currentCharacters?: number;
}

export function CharacterUsageDisplay({ usage, currentCharacters = 0 }: CharacterUsageDisplayProps) {
  const { charactersUsed, monthlyLimit, charactersRemaining } = usage;
  const percentage = getUsagePercentage(charactersUsed, monthlyLimit);
  const status = getUsageStatus(charactersUsed, monthlyLimit);
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center text-sm">
            ...
            <span className="font-bold text-lg">
              {formatCharacterCount(charactersUsed)} / {formatCharacterCount(monthlyLimit)}
            </span>
          </div>
          <Progress 
            value={percentage} 
            className={`h-3 transition-all duration-500 ${getStatusColor()}`}
            data-testid="progress-character-usage"
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center p-4 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-xl border border-green-200 dark:border-green-800">
            <div className="font-bold text-2xl text-green-600 mb-1">
              {formatCharacterCount(charactersRemaining)}
            </div>
            <div className="text-green-600/80 text-sm font-medium">Characters Remaining</div>
          </div>
          <div className="text-center p-4 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
            <div className="font-bold text-2xl text-blue-600 mb-1">
              {formatCharacterCount(charactersUsed)}
            </div>
            <div className="text-blue-600/80 text-sm font-medium">Characters Used</div>
          </div>
        </div>
          ...
          <div className="p-4 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-xl border border-purple-200 dark:border-purple-800">
            ...
            <div className="text-sm text-purple-700 dark:text-purple-300">
              <div className="flex justify-between items-center">
                <span>Characters to generate:</span>
                <span className="font-bold text-lg">{formatCharacterCount(currentCharacters)}</span>
              </div>
              {currentCharacters > charactersRemaining && (
                <div className="text-red-600 dark:text-red-400 mt-2 font-medium bg-red-50 dark:bg-red-900/20 p-2 rounded-lg">
                  ⚠️ Exceeds limit by {formatCharacterCount(currentCharacters - charactersRemaining)} characters
                </div>
              )}
            </div>
          </div>
